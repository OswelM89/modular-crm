...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'Users can view memberships in their organizations' AND schemaname='public' AND tablename='organization_members') THEN
    DROP POLICY "Users can view memberships in their organizations" ON public.organization_members;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'Organization owners can manage members' AND schemaname='public' AND tablename='organization_members') THEN
    DROP POLICY "Organization owners can manage members" ON public.organization_members;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'Users can insert themselves as members' AND schemaname='public' AND tablename='organization_members') THEN
    DROP POLICY "Users can insert themselves as members" ON public.organization_members;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'Users can view organizations they are members of' AND schemaname='public' AND tablename='organizations') THEN
    DROP POLICY "Users can view organizations they are members of" ON public.organizations;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'Organization owners can update their organizations' AND schemaname='public' AND tablename='organizations') THEN
