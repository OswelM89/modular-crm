...
export function Header({ activeSection, onSectionChange, user }: HeaderProps) {
  ...
  return (
    <header className="border-b border-border shadow-sm bg-gray-950">
                ...
                <div key={item.id} className="relative group">
                  <button
                    ...
                    className={`flex items-center justify-center w-10 h-10 rounded-md transition-all duration-200 ${
                      isActive
                        ? 'bg-[#77ff00] text-black shadow-sm'
                        : 'text-white hover:text-black hover:bg-[#77ff00]/60'
                    }`}
                  >
          ...
          <div className="flex items-center gap-4">
            ...
            {user && (
              <div className="relative user-dropdown">
                <div 
                  className="hidden sm:flex items-center gap-3 cursor-pointer transition-all duration-200"
                ...
                >
                  {user.avatar_url ? (
                    <img 
                      src={user.avatar_url} 
                      alt="Avatar" 
                      className="w-8 h-8 rounded-full ring-2 ring-[#77ff00]/40" 
                    />
                  ) : (
                    <div className="w-8 h-8 rounded-full bg-[#77ff00] text-black flex items-center justify-center text-sm font-medium ring-2 ring-[#77ff00]/40">
